...
export default function EditCommunityGuideModal({ open, guide, onClose, onSave }) {
  ...
  const [preview, setPreview] = useState(
    guide?.image?.filename ? `${baseUrl}/uploads/community-guides/${guide.image.filename}` : null
  );
  ...
  React.useEffect(() => {
    if (open && guide) {
      setForm({
        ...
        author: guide.author || "",
        tags: guide.tags || [],
        faq1Question: guide.faq1Question || "",
        faq1Answer: guide.faq1Answer || "",
        faq2Question: guide.faq2Question || "",
        faq2Answer: guide.faq2Answer || "",
        faq3Question: guide.faq3Question || "",
        faq3Answer: guide.faq3Answer || "",
        image: guide.image || null,
      });
      setPreview(guide?.image?.filename ? `${baseUrl}/uploads/community-guides/${guide.image.filename}` : null);
      setTagInput("");
      setError(null);
      setSuccess(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (hasNewImage) {
        ...
        requestData.append('guideId', guide._id);
        ...
        // Add FAQ fields
        requestData.append('faq1Question', form.faq1Question.trim());
        requestData.append('faq1Answer', form.faq1Answer.trim());
        requestData.append('faq2Question', form.faq2Question.trim());
        requestData.append('faq2Answer', form.faq2Answer.trim());
        requestData.append('faq3Question', form.faq3Question.trim());
        requestData.append('faq3Answer', form.faq3Answer.trim());
        ...
        requestData = {
          ...
          faq1Question: form.faq1Question.trim(),
          faq1Answer: form.faq1Answer.trim(),
          faq2Question: form.faq2Question.trim(),
          faq2Answer: form.faq2Answer.trim(),
          faq3Question: form.faq3Question.trim(),
          faq3Answer: form.faq3Answer.trim(),
          tags: form.tags
        };
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to update community guide');
      }
    } catch (err) {
      console.error('Error updating community guide:', err);
      setError(
        err.response?.data?.message || 
        err.message || 
        'Failed to update community guide. Please try again.'
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <form onSubmit={handleSubmit} className="edit-blog-form">
          ...
          {success && (
            <div className="success-message">
              ...
              <span className="success-text">Community guide updated successfully!</span>
            </div>
          )}
